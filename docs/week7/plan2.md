...
- [x] **결제 도메인 분산 락 동시성 제어 (Behavioral)**
  - [x] 결제 콜백과 스케줄러가 동시 실행되는 시나리오에 대한 실패하는 테스트 작성 (Red)
  - [x] `Redisson`을 활용한 분산 락(`payment:lock:{orderId}`) 기반의 동시성 갱신 방어 로직 구현 (Green)
- [x] **보상 트랜잭션 비동기화 및 아웃박스 발행 (Behavioral)**
  - [x] 기존 동기 방식 보상 로직(재고/쿠폰 복구) 인터페이스 분리 (Structural)
  - [x] 보상 트랜잭션 실패 시나리오 및 이벤트 발행 누락에 대한 실패하는 테스트 작성 (Red)
  - [x] 결제 실패 이벤트를 `Outbox` 테이블에 함께 저장하여 Kafka로 재처리 위임하도록 구현 (Green)
- [x] **PG 결제 콜백 API 구현 (Behavioral)**
  - [x] 결제 콜백 수신 및 정상 동작에 대한 실패하는 테스트 작성 (Red)
  - [x] 콜백 수신 후 이벤트 발행 최소 코드 구현 (Green)

## 2. Transactional Outbox 및 Kafka Producer 파이프라인

**목표:** 메인 트랜잭션과 통신(Kafka)을 안전하게 분리하고 데이터 유실을 100% 차단합니다.

- [x] **Outbox 엔티티 및 스키마 생성 (Structural)**
  - `OUTBOX_EVENTS` 테이블 (필드: id, aggregate_type, aggregate_id, payload, status 등) 및 리포지토리 구성
- [x] **주문/결제 이벤트의 Outbox 동시 저장 (Behavioral)**
  - 핵심 트랜잭션 내 이벤트가 `INIT` 상태로 DB에 올바르게 저장되는지 확인하는 테스트 작성 (Red)
  - 이벤트 리스너(동기 방식)를 통해 DB에 `Outbox` 데이터를 삽입하는 로직 구현 (Green)
- [x] **Kafka 비동기 전송 처리 (Behavioral)**
