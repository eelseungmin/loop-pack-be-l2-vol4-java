...
*   **스케줄러 통과 인원(배치 크기 N) 산정 근거:**
    ...
    *   **(가정 예시):** 시스템 파이프라인 중 가장 처리량이 낮은 구간(예: 외부 PG사 호출 지연 등)의 종합 병목 TPS가 30건/초 라고 가정한다. 결제 실패 시 재시도하는 유저들의 체류(TTL) 트래픽을 고려해 안전율 50%를 적용하면, 스케줄러는 1초마다 **N=15명**을 통과시킨다.
    *   **검증 필수:** 이는 단순 이론값이므로, 실제 구현 후 **k6 부하 테스트를 통해 End-to-End 결제 시나리오(주문+결제)의 종합 병목 TPS를 직접 측정하여 N 값을 최종 튜닝**해야 한다.
*   **대기 시간 계산 (고정 처리량 방식):**
    *   예상 대기 시간은 `(내 앞의 대기 인원 수) / (시스템의 초당 고정 처리 허용량 N)` 으로 단순 계산하여 반환한다.
