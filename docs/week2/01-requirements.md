...
*   **좋아요 수 관리 (Eventual Consistency):** 상품 목록의 '좋아요 많은 순' 정렬 조회를 극대화하기 위해 `PRODUCT` 테이블에 `like_count`를 비정규화하되, 실시간 강한 정합성 대신 **이벤트 기반의 최종 일관성(Eventual Consistency)**을 적용한다.
*   **좋아요 로직과 집계 트랜잭션 분리:** 
    *   좋아요 데이터(Like) 추가/삭제 자체는 사용자의 요청 트랜잭션 안에서 즉시 완료하여 응답 속도를 높인다.
    *   `like_count` 증가/감소는 `ApplicationEvent`(`@TransactionalEventListener(AFTER_COMMIT)`)를 통해 분리된 비동기 트랜잭션에서 처리한다.
*   **실패 보정 (Guaranteed Delivery):** 비동기 좋아요 집계 업데이트 실패 시, 실패 이벤트를 별도의 테이블(Outbox)에 기록하고 배치 스케줄러가 이를 주기적으로 재처리하여 정확한 통계 수치를 최종 보장한다.
...
### 2.9 장애 격리 및 서킷 브레이커 (Resilience)
...
### 2.10 이벤트 기반 부가 로직 분리 (ApplicationEvent)
...
*   **주문/결제 완료 알림 (Notification):**
    *   결제 성공 후 유저에게 알림을 발송하는 부가 로직은 `@TransactionalEventListener(phase = AFTER_COMMIT)`을 사용하여 결제 트랜잭션이 안전하게 커밋된 이후에 비동기(`@Async`)로 실행되게 분리한다.
    *   **알림 보장 정책 (Guaranteed Delivery):** 알림 발송 중 외부 API 장애가 발생하면 실패 내역을 DB에 기록해두고 배치 스케줄러가 재발송을 시도하여 도달률을 보장한다.
